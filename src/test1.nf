...
params.in_dir_blue = "/projects/bgmp/shared/groups/2024/novel-fluor/malm/mini_illum/blue"
params.in_dir_red = "/projects/bgmp/shared/groups/2024/novel-fluor/malm/mini_illum/red"
...
params.counts_script = "/projects/bgmp/shared/groups/2024/novel-fluor/malm/BGMP2024_novel-fluor/src/generate_counts_better.py"
...
workflow {
    ...
    // Trim reads
    trimmed_blue_ch = trim_reads_blue(merged_blue_ch)
    ...
    counts_blue_ch = generate_counts_blue(counts_blue_input_ch)
    counts_red_ch = generate_counts_red(counts_red_input_ch)
...
process trim_reads_blue {
    ...
    output:
    tuple val(key), path ("*"), emit: trimmed_blue
...
process trim_reads_red {
    ...
    output:
    tuple val(key), path ("*"), emit: trimmed_red
...
process generate_counts_blue {
    ...
    input:
    file("${params.out_dir_trim_blue}/*.fastq.gz")

    output:
    path "blue_counts_FINAL.tsv", emit: blue_counts

    script:  
    """
    /projects/bgmp/shared/groups/2024/novel-fluor/malm/BGMP2024_novel-fluor/src/generate_counts_better.py \
        --in_dir "${params.out_dir_trim_blue}" \
        --out_file "blue_counts_FINAL.tsv"
    """
}
